...
namespace BBG
{
	public class ScreenManager : SingletonComponent<ScreenManager>
	{
		...
		public void Back()
		{
			...
			cam.transform.position = new Vector3(0f, 5.71f, 0f);
			cam2.GetComponent<Camera>().targetTexture = Resources.Load("Lvl1") as RenderTexture;
			canvas.GetComponent<Canvas>().renderMode = RenderMode.ScreenSpaceCamera;
			if (Application.systemLanguage == SystemLanguage.Russian)
			{
				SoundManager.Instance.Stop("bkg-music");
				SoundManager.Instance.Stop("galaxy");
				SoundManager.Instance.Stop("lvl3");
				SoundManager.Instance.Stop("lvl4");
				SoundManager.Instance.Stop("lvl5");
